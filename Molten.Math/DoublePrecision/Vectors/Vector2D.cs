...
public partial struct Vector2D : IFormattable, ISignedVector<Vector2D, double>, IEquatable<Vector2D>
{
	...
	/// <summary>A fixed array mapped to the same memory space as the individual <see cref="Vector2D"/> components.</summary>
	[IgnoreDataMember]
	[FieldOffset(0)]
...
#region Constructors
	...
	public unsafe Vector2D(double[] values)
	{
		if (values == null)
			throw new ArgumentNullException("values");
		if (values.Length < 2)
			throw new ArgumentOutOfRangeException("values", "There must be at least 2 input values for Vector2D.");

		fixed (double* src = values)
		{
			fixed (double* dst = Values)
				Unsafe.CopyBlock(src, dst, (sizeof(double) * 2));
		}
	}
	...
	public unsafe Vector2D(double* ptrValues)
	{
		...
		fixed (double* dst = Values)
			Unsafe.CopyBlock(ptrValues, dst, (sizeof(double) * 2));
	}
...
#region Tuples
    ...
    /// <summary>
    /// Constructs a <see cref="Vector2D"/> from 2 component values.
    /// </summary>
...
#region Indexers
	...
	/// <value>The value of the <see cref="Vector2D"/> component, depending on the index.</value>
	...
	public unsafe double this[int index]
	{
		...
		{
			if(index < 0 || index > 1)
				throw new IndexOutOfRangeException("index for Vector2D must be between 0 and 1, inclusive.");
