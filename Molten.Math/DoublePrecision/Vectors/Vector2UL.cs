...
namespace Molten.DoublePrecision
{
	...
	public partial struct Vector2UL : IFormattable, IUnsignedVector<Vector2UL, ulong>, IEquatable<Vector2UL>
	{
		...
		/// <summary>A fixed array mapped to the same memory space as the individual <see cref="Vector2UL"/> components.</summary>
		[IgnoreDataMember]
		[FieldOffset(0)]
        ...
        /// </summary>
        public bool IsZero => X == 0UL && Y == 0UL;
...
#region Constructors
		...
		public unsafe Vector2UL(ulong[] values)
		{
			if (values == null)
				throw new ArgumentNullException("values");
			if (values.Length < 2)
				throw new ArgumentOutOfRangeException("values", "There must be at least 2 input values for Vector2UL.");

			fixed (ulong* src = values)
			{
				fixed (ulong* dst = Values)
					Unsafe.CopyBlock(src, dst, (sizeof(ulong) * 2));
			}
		}
		...
		public unsafe Vector2UL(ulong* ptrValues)
		{
			...
			fixed (ulong* dst = Values)
				Unsafe.CopyBlock(ptrValues, dst, (sizeof(ulong) * 2));
		}
...
#region Tuples
        ...
        /// <summary>
        /// Constructs a <see cref="Vector2UL"/> from 2 component values.
        /// </summary>
        /// <param name="tuple">The 2-component tuple containing the values.</param>
        public static implicit operator Vector2UL((ulong x, ulong y) tuple)
        {
            return new Vector2UL(tuple.x, tuple.y);
        }
#endregion
...
#region Static Methods
        ...
        public static Vector2UL Reflect(Vector2UL vector, Vector2UL normal)
        {
...
#region Indexers
		...
		/// <value>The value of the <see cref="Vector2UL"/> component, depending on the index.</value>
		...
		public unsafe ulong this[int index]
		{
			...
			{
				if(index < 0 || index > 1)
					throw new IndexOutOfRangeException("index for Vector2UL must be between 0 and 1, inclusive.");
