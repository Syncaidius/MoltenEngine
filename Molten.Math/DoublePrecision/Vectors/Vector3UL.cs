...
namespace Molten.DoublePrecision
{
	...
	public partial struct Vector3UL : IFormattable, IUnsignedVector<Vector3UL, ulong>, IEquatable<Vector3UL>
	{
		...
		/// <summary>A fixed array mapped to the same memory space as the individual <see cref="Vector3UL"/> components.</summary>
		[IgnoreDataMember]
		[FieldOffset(0)]
		...
		public unsafe Vector3UL(ulong[] values)
		{
			...
			fixed (ulong* src = values)
			{
				fixed (ulong* dst = Values)
					Unsafe.CopyBlock(src, dst, (sizeof(ulong) * 3));
			}
		}
...
#region Indexers
		...
		/// <value>The value of the <see cref="Vector3UL"/> component, depending on the index.</value>
		...
		public unsafe ulong this[int index]
		{
			...
			{
				if(index < 0 || index > 2)
					throw new IndexOutOfRangeException("index for Vector3UL must be between 0 and 2, inclusive.");
