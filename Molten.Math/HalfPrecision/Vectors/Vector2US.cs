...
namespace Molten.HalfPrecision
{
	...
	public partial struct Vector2US : IFormattable, IUnsignedVector<Vector2US, ushort>, IEquatable<Vector2US>
	{
		...
		/// <summary>A fixed array mapped to the same memory space as the individual <see cref="Vector2US"/> components.</summary>
		[IgnoreDataMember]
		[FieldOffset(0)]
        ...
        /// </summary>
        public bool IsZero => X == (ushort)0 && Y == (ushort)0;
...
#region Constructors
		...
		public unsafe Vector2US(ushort[] values)
		{
			if (values == null)
				throw new ArgumentNullException("values");
			if (values.Length < 2)
				throw new ArgumentOutOfRangeException("values", "There must be at least 2 input values for Vector2US.");

			fixed (ushort* src = values)
			{
				fixed (ushort* dst = Values)
					Unsafe.CopyBlock(src, dst, (sizeof(ushort) * 2));
			}
		}
		...
		public unsafe Vector2US(ushort* ptrValues)
		{
			...
			fixed (ushort* dst = Values)
				Unsafe.CopyBlock(ptrValues, dst, (sizeof(ushort) * 2));
		}
...
#region Tuples
        ...
        /// <summary>
        /// Constructs a <see cref="Vector2US"/> from 2 component values.
        /// </summary>
        /// <param name="tuple">The 2-component tuple containing the values.</param>
        public static implicit operator Vector2US((ushort x, ushort y) tuple)
        {
            return new Vector2US(tuple.x, tuple.y);
        }
#endregion
...
#region Indexers
		...
		/// <value>The value of the <see cref="Vector2US"/> component, depending on the index.</value>
		...
		public unsafe ushort this[int index]
		{
			...
			{
				if(index < 0 || index > 1)
					throw new IndexOutOfRangeException("index for Vector2US must be between 0 and 1, inclusive.");
