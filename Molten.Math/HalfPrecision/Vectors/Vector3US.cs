...
namespace Molten.HalfPrecision
{
	...
	public partial struct Vector3US : IFormattable, IUnsignedVector<Vector3US, ushort>, IEquatable<Vector3US>
	{
		...
		/// <summary>A fixed array mapped to the same memory space as the individual <see cref="Vector3US"/> components.</summary>
		[IgnoreDataMember]
		[FieldOffset(0)]
		public unsafe fixed ushort Values[3];

        /// <summary>
        /// Gets a value indicting whether this vector is zero
        /// </summary>
        public bool IsZero => X == (ushort)0 && Y == (ushort)0 && Z == (ushort)0;
...
#region Constructors
		...
		public unsafe Vector3US(ushort[] values)
		{
			if (values == null)
				throw new ArgumentNullException("values");
			if (values.Length < 3)
				throw new ArgumentOutOfRangeException("values", "There must be at least 3 input values for Vector3US.");

			fixed (ushort* src = values)
			{
				fixed (ushort* dst = Values)
					Unsafe.CopyBlock(src, dst, (sizeof(ushort) * 3));
			}
		}
		...
		public unsafe Vector3US(ushort* ptrValues)
		{
			...
			fixed (ushort* dst = Values)
				Unsafe.CopyBlock(ptrValues, dst, (sizeof(ushort) * 3));
		}

		///<summary>Creates a new instance of <see cref="Vector3US"/>, using a <see cref="Vector2US"/> to populate the first 2 components.</summary>
...
#region Tuples
        ...
        /// <summary>
        /// Constructs a <see cref="Vector3US"/> from 3 component values.
        /// </summary>
        /// <param name="tuple">The 3-component tuple containing the values.</param>
        public static implicit operator Vector3US((ushort x, ushort y, ushort z) tuple)
        {
            return new Vector3US(tuple.x, tuple.y, tuple.z);
        }
#endregion
...
#region To-String
		/// <summary>
        /// Returns a <see cref="string"/> that represents this <see cref="Vector3US"/>.
        /// </summary>
        /// <param name="format">The format.</param>
        /// <returns>
        /// A <see cref="string"/> that represents this <see cref="Vector3US"/>.
        /// </returns>
...
#region Static Methods
        ...
        public static Vector3US Reflect(Vector3US vector, Vector3US normal)
        {
...
#region Indexers
		...
		/// <value>The value of the <see cref="Vector3US"/> component, depending on the index.</value>
		...
		public unsafe ushort this[int index]
		{
			...
			{
				if(index < 0 || index > 2)
					throw new IndexOutOfRangeException("index for Vector3US must be between 0 and 2, inclusive.");
