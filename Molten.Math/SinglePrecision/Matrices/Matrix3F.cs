...
namespace Molten
{
	...
	public partial struct Matrix3F : IEquatable<Matrix3F>, IFormattable, ITransposableMatrix<Matrix3F, Matrix3F>, IUniformMatrix<float>
    {
		...
		/// <value>The value of the <see cref="Matrix3F"/> component, depending on the index.</value>
		...
		public unsafe float this[int index]
		{
			...
			{
				if(index < 0 || index > 8)
					throw new IndexOutOfRangeException("index for Matrix3F must be between 0 and 8, inclusive.");
		...
		public unsafe float this[int row, int column]
		{
			...
			{
				if(row < 0 || row > 8)
					throw new IndexOutOfRangeException("row for Matrix3F must be between 0 and 8, inclusive.");

				if(column < 0 || column > 8)
					throw new IndexOutOfRangeException("column for Matrix3F must be between 0 and 8, inclusive.");
