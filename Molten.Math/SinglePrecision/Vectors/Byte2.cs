...
namespace Molten
{
	...
	public partial struct Byte2 : IFormattable, IUnsignedVector<Byte2, byte>, IEquatable<Byte2>
	{
		...
		/// <summary>A fixed array mapped to the same memory space as the individual <see cref="Byte2"/> components.</summary>
		[IgnoreDataMember]
		[FieldOffset(0)]
...
#region Constructors
		...
		public unsafe Byte2(byte[] values)
		{
			...
			fixed (byte* src = values)
			{
				fixed (byte* dst = Values)
					Unsafe.CopyBlock(src, dst, (sizeof(byte) * 2));
			}
		}
		...
		public unsafe Byte2(byte* ptrValues)
		{
			...
			fixed (byte* dst = Values)
				Unsafe.CopyBlock(ptrValues, dst, (sizeof(byte) * 2));
		}
...
#region Indexers
		...
		/// <value>The value of the <see cref="Byte2"/> component, depending on the index.</value>
		...
		public unsafe byte this[int index]
		{
			...
			{
				if(index < 0 || index > 1)
					throw new IndexOutOfRangeException("index for Byte2 must be between 0 and 1, inclusive.");
