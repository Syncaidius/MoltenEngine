...
public partial struct SByte4 : IFormattable, ISignedVector<SByte4, sbyte>, IEquatable<SByte4>
{
	...
	/// <summary>A fixed array mapped to the same memory space as the individual <see cref="SByte4"/> components.</summary>
	[IgnoreDataMember]
	[FieldOffset(0)]
...
#region Constructors
	...
	public unsafe SByte4(sbyte[] values)
	{
		...
		fixed (sbyte* src = values)
		{
			fixed (sbyte* dst = Values)
				Unsafe.CopyBlock(src, dst, (sizeof(sbyte) * 4));
		}
	}
	...
	public unsafe SByte4(sbyte* ptrValues)
	{
		...
		fixed (sbyte* dst = Values)
			Unsafe.CopyBlock(ptrValues, dst, (sizeof(sbyte) * 4));
	}
...
#region Indexers
	...
	/// <value>The value of the <see cref="SByte4"/> component, depending on the index.</value>
	...
	public unsafe sbyte this[int index]
	{
		...
		{
			if(index < 0 || index > 3)
				throw new IndexOutOfRangeException("index for SByte4 must be between 0 and 3, inclusive.");
