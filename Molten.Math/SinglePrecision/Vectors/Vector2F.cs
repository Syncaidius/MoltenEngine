...
public partial struct Vector2F : IFormattable, ISignedVector<Vector2F, float>, IEquatable<Vector2F>
{
	...
	/// <summary>A fixed array mapped to the same memory space as the individual <see cref="Vector2F"/> components.</summary>
	[IgnoreDataMember]
	[FieldOffset(0)]
...
#region Constructors
	...
	public unsafe Vector2F(float[] values)
	{
		if (values == null)
			throw new ArgumentNullException("values");
		if (values.Length < 2)
			throw new ArgumentOutOfRangeException("values", "There must be at least 2 input values for Vector2F.");

		fixed (float* src = values)
		{
			fixed (float* dst = Values)
				Unsafe.CopyBlock(src, dst, (sizeof(float) * 2));
		}
	}
...
#region Tuples
    ...
    /// <summary>
    /// Constructs a <see cref="Vector2F"/> from 2 component values.
    /// </summary>
    /// <param name="tuple">The 2-component tuple containing the values.</param>
    public static implicit operator Vector2F((float x, float y) tuple)
    {
        return new Vector2F(tuple.x, tuple.y);
    }
#endregion
...
#region Indexers
	...
	/// <value>The value of the <see cref="Vector2F"/> component, depending on the index.</value>
	...
	public unsafe float this[int index]
	{
		...
		{
			if(index < 0 || index > 1)
				throw new IndexOutOfRangeException("index for Vector2F must be between 0 and 1, inclusive.");
