...
namespace Molten
{
    ...
    [DataContract]
	public partial struct Vector2I : IFormattable, ISignedVector<Vector2I, int>, IEquatable<Vector2I>
	{
		...
		/// <summary>A fixed array mapped to the same memory space as the individual <see cref="Vector2I"/> components.</summary>
		[IgnoreDataMember]
		[FieldOffset(0)]
...
#region Constructors
		...
		public unsafe Vector2I(int[] values)
		{
			if (values == null)
				throw new ArgumentNullException("values");
			if (values.Length < 2)
				throw new ArgumentOutOfRangeException("values", "There must be at least 2 input values for Vector2I.");

			fixed (int* src = values)
			{
				fixed (int* dst = Values)
					Unsafe.CopyBlock(src, dst, (sizeof(int) * 2));
			}
		}
		...
		public unsafe Vector2I(int* ptrValues)
		{
			...
			fixed (int* dst = Values)
				Unsafe.CopyBlock(ptrValues, dst, (sizeof(int) * 2));
		}
...
#region Tuples
        ...
        /// <summary>
        /// Constructs a <see cref="Vector2I"/> from 2 component values.
        /// </summary>
