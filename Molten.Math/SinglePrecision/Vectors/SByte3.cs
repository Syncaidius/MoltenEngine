...
namespace Molten
{
	...
	public partial struct SByte3 : IFormattable, ISignedVector<SByte3, sbyte>, IEquatable<SByte3>
	{
		...
		/// <summary>A fixed array mapped to the same memory space as the individual <see cref="SByte3"/> components.</summary>
		[IgnoreDataMember]
		[FieldOffset(0)]
		public unsafe fixed sbyte Values[3];
		...
		public unsafe SByte3(sbyte[] values)
		{
			...
			fixed (sbyte* src = values)
			{
				fixed (sbyte* dst = Values)
					Unsafe.CopyBlock(src, dst, (sizeof(sbyte) * 3));
			}
		}
...
#region Static Methods
        ...
        public static SByte3 Reflect(SByte3 vector, SByte3 normal)
        {
...
#region Indexers
		...
		/// <value>The value of the <see cref="SByte3"/> component, depending on the index.</value>
		...
		public unsafe sbyte this[int index]
		{
			...
			{
				if(index < 0 || index > 2)
					throw new IndexOutOfRangeException("index for SByte3 must be between 0 and 2, inclusive.");
