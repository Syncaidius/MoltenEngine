...
public partial struct SByte2 : IFormattable, ISignedVector<SByte2, sbyte>, IEquatable<SByte2>
{
	...
	/// <summary>A fixed array mapped to the same memory space as the individual <see cref="SByte2"/> components.</summary>
	[IgnoreDataMember]
	[FieldOffset(0)]
    ...
    /// </summary>
    public bool IsZero => X == (sbyte)0 && Y == (sbyte)0;
...
#region Constructors
	...
	public unsafe SByte2(sbyte[] values)
	{
		if (values == null)
			throw new ArgumentNullException("values");
		if (values.Length < 2)
			throw new ArgumentOutOfRangeException("values", "There must be at least 2 input values for SByte2.");

		fixed (sbyte* src = values)
		{
			fixed (sbyte* dst = Values)
				Unsafe.CopyBlock(src, dst, (sizeof(sbyte) * 2));
		}
	}
	...
	public unsafe SByte2(sbyte* ptrValues)
	{
		...
		fixed (sbyte* dst = Values)
			Unsafe.CopyBlock(ptrValues, dst, (sizeof(sbyte) * 2));
	}
...
#region Tuples
    ...
    /// <summary>
    /// Constructs a <see cref="SByte2"/> from 2 component values.
    /// </summary>
    /// <param name="tuple">The 2-component tuple containing the values.</param>
    public static implicit operator SByte2((sbyte x, sbyte y) tuple)
    {
        return new SByte2(tuple.x, tuple.y);
    }
#endregion
...
#region Indexers
	...
	/// <value>The value of the <see cref="SByte2"/> component, depending on the index.</value>
	...
	public unsafe sbyte this[int index]
	{
		...
		{
			if(index < 0 || index > 1)
				throw new IndexOutOfRangeException("index for SByte2 must be between 0 and 1, inclusive.");
