...
public partial struct Byte4 : IFormattable, IUnsignedVector<Byte4, byte>, IEquatable<Byte4>
{
	...
	/// <summary>A fixed array mapped to the same memory space as the individual <see cref="Byte4"/> components.</summary>
	[IgnoreDataMember]
	[FieldOffset(0)]
	...
	public unsafe Byte4(byte[] values)
	{
		...
		fixed (byte* src = values)
		{
			fixed (byte* dst = Values)
				Unsafe.CopyBlock(src, dst, (sizeof(byte) * 4));
		}
	}
...
#region Indexers
	...
	/// <value>The value of the <see cref="Byte4"/> component, depending on the index.</value>
	...
	public unsafe byte this[int index]
	{
		...
		{
			if(index < 0 || index > 3)
				throw new IndexOutOfRangeException("index for Byte4 must be between 0 and 3, inclusive.");
