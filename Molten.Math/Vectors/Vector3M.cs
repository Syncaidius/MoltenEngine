...
#region Static Methods
        ...
        /// <param name="vec">The Vector3M to be truncated.</param>
        /// <returns></returns>
        public static Vector3M Truncate(Vector3M value)
        {
            return new Vector3M()
            {
				X = (Math.Abs(value.X) - 0.0001M < 0) ? 0 : value.X,
				Y = (Math.Abs(value.Y) - 0.0001M < 0) ? 0 : value.X,
				Z = (Math.Abs(value.Z) - 0.0001M < 0) ? 0 : value.X,
            };
        }
#endregion
