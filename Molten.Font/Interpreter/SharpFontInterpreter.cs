...
namespace Molten.Font
{
    ...
    internal class SharpFontInterpreter
    {
        ...
        public SharpFontInterpreter(int maxStack, int maxStorage, int maxFunctions, int maxInstructionDefs, int maxTwilightPoints)
        {
            ...
            twilight = new Zone(new GlyphPoint[maxTwilightPoints], isTwilight: true);
        }
        ...
        public void HintGlyph(GlyphPoint[] glyphPoints, ushort[] contours, byte[] instructions)
        {
            ...
            while (!stream.Done)
            {
                ...
                switch (opcode)
                {
                    ...
                    case OpCode.SCFS:
                        {
                            ...
                            if (zp2.IsTwilight)
                                zp2.Original[index].Point = zp2.Current[index].Point;
                        }
                        break;
                    case OpCode.MD0:
                        {
                    ...
                    case OpCode.FLIPPT:
                        {
                            ...
                            {
                                var index = stack.Pop();
                                points.Current[index].IsOnCurve = !points.Current[index].IsOnCurve;
                            }
                            state.Loop = 1;
                        }
                        break;
                    case OpCode.FLIPRGON:
                        {
                            ...
                            for (int i = stack.Pop(); i <= end; i++)
                                ...
                                points.Current[i].IsOnCurve = true;
                        }
                        break;
                    case OpCode.FLIPRGOFF:
                        {
                            ...
                            for (int i = stack.Pop(); i <= end; i++)
                                ...
                                points.Current[i].IsOnCurve = false;
                        }
                        break;
                    case OpCode.SHP0:
                    case OpCode.SHP1:
                    ...
                    case OpCode.SHC1:
                        {
                            ...
                            for (int i = start; i < count; i++)
                            {
                                ...
                                {
                                    zp2.Current[i].Point += displacement;
                    ...
                    case OpCode.MIAP1:
                        {
                            ...
                            if (zp0.IsTwilight)
                            {
                                ...
                                zp0.Original[pointIndex].Point = original;
                                zp0.Current[pointIndex].Point = original;
                            }
                    ...
                    case OpCode.MSIRP1:
                        {
                            ...
                            if (zp1.IsTwilight)
                            {
                                zp1.Original[pointIndex].Point = zp0.Original[state.Rp0].Point + targetDistance * state.Freedom / fdotp;
                                zp1.Current[pointIndex].Point = zp1.Original[pointIndex].Point;
                            }
                    ...
                    case OpCode.IUP1:
                        ...
                        //    GlyphPoint[] currentPnts = points.Current;
                        //    GlyphPoint[] originalPnts = points.Original;
                        ...
                        unsafe
                        {
                            ...
                            fixed (GlyphPoint* currentPtr = points.Current)
                            fixed (GlyphPoint* originalPtr = points.Original)
                            {
                                // opcode controls whether we care about X or Y direction
                                // do some pointer trickery so we can operate on the
                                // points in a direction-agnostic manner
                    ...
                    case OpCode.ISECT:
                        {
                            ...
                            if (Math.Abs(den) <= Epsilon)
                            {
                                ...
                                zp2.Current[index].Point = (a0 + a1 + b0 + b1) / 4;
                            }
                            else
                            {
                    ...
                    case OpCode.DIV:
                        {
                            var b = stack.Pop();
                            if (b == 0)
                                throw new FontInterpreterException("Division by zero.");
                    ...
                    case OpCode.FDEF:
                        {
                            if (!allowFunctionDefs || inFunction)
                                throw new FontInterpreterException("Can't define functions here.");
                    ...
                    case OpCode.ENDF:
                        {
                            if (!inFunction)
                                throw new FontInterpreterException("Found invalid ENDF marker outside of a function definition.");
                            return;
                        }
                    case OpCode.CALL:
                    case OpCode.LOOPCALL:
                        {
                            ...
                            if (callStackSize > MaxCallStack)
                                throw new FontInterpreterException("Stack overflow; infinite recursion?");
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            if (index > instructionDefs.Length || !instructionDefs[index].IsValid)
                                throw new FontInterpreterException("Unknown opcode in font program.");
        ...
        Zone GetZoneFromStack()
        {
            switch (stack.Pop())
            {
                ...
                default: throw new FontInterpreterException("Invalid zone pointer.");
            }
        }
        ...
        void SetSuperRound(float period)
        {
            ...
            switch (mode & 0xC0)
            {
                ...
                default: throw new FontInterpreterException("Unknown rounding period multiplier.");
            }
        ...
        void MoveIndirectRelative(int flags)
        {
            ...
            if (zp1.IsTwilight)
            {
                ...
                zp1.Original[pointIndex].Point = initialValue;
                zp1.Current[pointIndex].Point = initialValue;
            }
        ...
        void ShiftPoints(Vector2F displacement)
        {
            ...
            {
                var pointIndex = stack.Pop();
                zp2.Current[pointIndex].Point += displacement;
        ...
        static void InterpolatePointsXAxis(GlyphPoint[] current, GlyphPoint[] original, int start, int end, int ref1, int ref2)
        {
            if (start > end)
                return;
        ...
        static void InterpolatePointsYAxis(GlyphPoint[] current, GlyphPoint[] original, int start, int end, int ref1, int ref2)
        {
            if (start > end)
                return;
        ...
        struct InstructionStream
        {
            ...
            {
                if (Done)
                    throw new FontInterpreterException();
        ...
        class ExecutionStack
        {
            ...
            {
                if (count == s.Length)
                    throw new FontInterpreterException("Instruction stack is full.");
                s[count++] = value;
            }
            ...
            {
                if (count == 0)
                    throw new FontInterpreterException("Instruction stack is empty.");
                return s[--count];
            }
        ...
        struct Zone
        {
            public GlyphPoint[] Current;
            public GlyphPoint[] Original;
            public TouchState[] TouchState;
            public bool IsTwilight;

            public Zone(GlyphPoint[] points, bool isTwilight)
            {
                IsTwilight = isTwilight;
                Current = points;
                Original = (GlyphPoint[])points.Clone();
