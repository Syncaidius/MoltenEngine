...
namespace Molten.UI
{
    public partial class UITextArea : UICompoundComponent
    {
        ...
        public void Copy()
        {
            ...
            _engine.Input.Clipboard.SetText(str);
        }
        ...
        public void Paste()
        {
            ...
            if (_engine.Input.Clipboard.ContainsText())
                InsertText(_caret, _engine.Input.Clipboard.GetText());
        }
        ...
        protected override void OnUpdate(Timing time)
        {
            ...
            if (_ui.Focused == this)
            {
                ...
                else if (_ui.Keyboard.IsTapped(Key.Up))
                {
                    GoUpAndAlign();
                }
                else if (_ui.Keyboard.IsTapped(Key.Down))
                {
                    GoDownAndAlign();
                }
                else if (_ui.Keyboard.IsTapped(Key.PageUp))
                {
                    int pageHeight = _lineCapacity * (int)_lines[0].textObject.GetSize().Y;
                    _vBar.Scroll(-pageHeight);
                }
                else if (_ui.Keyboard.IsTapped(Key.PageDown))
                {
                    int pageHeight = _lineCapacity * (int)_lines[0].textObject.GetSize().Y;
                    _vBar.Scroll(pageHeight);
                }
                ...
                else if (_ui.Keyboard.IsPressed(Key.Up))
                {
                    DoSystemKeyAction(time, GoUpAndAlign);
                }
                else if (_ui.Keyboard.IsPressed(Key.Down))
                {
                    DoSystemKeyAction(time, GoDownAndAlign);
                }
                else if (_ui.Keyboard.IsPressed(Key.PageUp))
                {
                    DoPageKey(time, -1);
                }
                else if (_ui.Keyboard.IsPressed(Key.PageDown))
                {
                    DoPageKey(time, 1);
                }
                else
