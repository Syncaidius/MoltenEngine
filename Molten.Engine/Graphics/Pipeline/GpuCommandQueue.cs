...
public abstract class GpuCommandQueue : EngineObject
{
    ...
    public virtual void Begin(GpuCommandListFlags flags = GpuCommandListFlags.None)
    {
#if DEBUG
        if (DrawInfo.Began)
            throw new GpuCommandQueueException(this, $"{nameof(GpuCommandList)}: End() must be called before the next Begin() call.");
#endif

        DrawInfo.Began = true;
    }

    public virtual GpuCommandList End()
    {
#if DEBUG
        if (!DrawInfo.Began)
            throw new GpuCommandQueueException(this, $"{nameof(GpuCommandList)}: BeginDraw() must be called before EndDraw().");
#endif
    ...
    protected GpuBindResult ApplyState(Shader shader, QueueValidationMode mode, Action callback)
    {
        ...
        if (!DrawInfo.Began)
            throw new GpuCommandQueueException(this, $"{GetType().Name}: BeginDraw() must be called before calling {nameof(Draw)}()");
    ...
    /// <summary>Gets the profiler bound to the current <see cref="GpuCommandQueue"/>.</summary>
    public GraphicsQueueProfiler Profiler { get; }

    /// <summary>
    /// Gets the current <see cref="GpuCommandList"/> of the current <see cref="GpuCommandQueue"/>.
    /// </summary>
    protected abstract GpuCommandList Cmd { get; }
