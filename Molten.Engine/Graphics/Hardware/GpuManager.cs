...
/// </summary>
public abstract class GpuManager : EngineObject
{
    ...
    public void Initialize(RenderService renderer, GraphicsSettings settings)
    {
        ...
        GpuDevice primaryDevice = ValidateAdapterSettings(settings);
    ...
    private void LogAdapter(GpuDevice adapter, int index)
    {
    ...
    /// <summary>Invoked when the current <see cref="GpuManager"/> is being initialized..</summary>
    ...
    /// <summary>
    /// Gets a list of all detected <see cref="GpuDevice"/> on the current machine.
    /// </summary>
    public abstract IReadOnlyList<GpuDevice> Devices { get; }
    ...
    /// <param name="adapters">The list in which to add compatibel <see cref="GpuDevice"/> instances.</param>
    public abstract void GetCompatibleAdapters(GpuCapabilities capabilities, List<GpuDevice> adapters);

    /// <summary>Gets the system's default display adapter.</summary>
    public abstract GpuDevice DefaultDevice { get; }

    /// <summary>Gets or sets the primary <see cref="GpuDevice"/>. The primary device will handle the majority of main rendering tasks, 
    /// while minor tasks that do not directly affect render performance can be offloaded to secondary ones.</summary>
    public abstract GpuDevice PrimaryDevice { get; set; }

    /// <summary>
    /// Gets the <see cref="GraphicsSettings"/> that the current <see cref="GpuManager"/> was initialized and bound to.
    /// </summary>
    public GraphicsSettings Settings { get; private set; }

    /// <summary>
    /// Getts the <see cref="Logger"/> that the current <see cref="GpuManager"/> uses for outputting information.
    /// </summary>
    public Logger Log { get; private set; }

    /// <summary>
    /// Attempts to find a <see cref="GpuDevice"/> with an ID matching the one provided.
    /// </summary>
    /// <param name="id">The <see cref="DeviceID"/> to match.</param>
    /// <returns>A matching <see cref="GpuDevice"/>, or null if no match was found.</returns>
    public GpuDevice this[DeviceID id]
    {
        get
        {
            foreach (GpuDevice devices in Devices)
            {
                if (devices.ID == id)
                    return devices;
            }
